[本项目参考](https://github.com/Ice-Hazymoon/openai-scf-proxy.git)

***

# 猴子也能学会的腾讯云函数搭建 Google Gemini 国内代理教程
...
注意：本教程只是教你搭建一个 Google Gemini 的代理，需要配合其他类似软件使用，直接访问会出现 404 的错误。
...
- 函数类型：Web函数
- 函数名称：gemini-pro-proxy（也可以随便取个名字）
- 地域：香港（也可以是中国之外的任何国家）
...
使用的时候 "/release" 部分删除与否都可以

例如：`https://service-aaaaa.hk.apigw.tencentcs.com/release/`

或者：`https://service-aaaaa.hk.apigw.tencentcs.com/`

## 如何使用

### 第三方项目

你可以在任何支持配置 Google Gemini 域名的软件中使用这个服务，例如在 CatGPT 中配置：

进入：[https://catgpt.run/](https://catgpt.run/)

点击右上角的🔑图标，在接口地址中输入这个地址，点击保存即可

愉快的与 Google Gemini 一起冲浪吧~

## 自托管

```
git clone https://github.com/donturman/gemini-pro-proxy.git
cd gemini-pro-proxy
npm install
npm run start
```
